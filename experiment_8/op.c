...
int main()
{
    ...
    while (1)
    {
        ...
        {
            while (top >= 2 && isOperator(stack[top - 1]) && (precedence(stack[top - 1]) > precedence(str[i]) || (precedence(stack[top - 1]) == precedence(str[i]) && str[i] != '^')))
            {
                reduce();
            }
