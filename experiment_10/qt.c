...
void printQuadruple(){
    ...
    for(int i=0; i<tacc; i++){
        printf("%d\t%c\t%s\t%s\t%s\n", i+1, tac[i].op, tac[i].arg1, tac[i].arg2, tac[i].result);
    }
}
...
void printTriple(){
    ...
    for(int i=0; i<tacc; i++){
        if(strlen(tac[i].arg1)>1){
            sprintf(tac[i].arg1, "(%d)", (int)(tac[i].arg1[1]-'0')-1);
        }
        if(strlen(tac[i].arg2)>1){
            sprintf(tac[i].arg2, "(%d)", (int)(tac[i].arg2[1]-'0')-1);
        }
